...
# Ask questions
REPLACEME_SUBDOMAIN=$(whiptail --inputbox --title "Replaceme Subdomain" "Please set the subdomain to use for Replaceme" 20 60 "replaceme" 3>&1 1>&2 2>&3)
exitstatus=$?
if [ $exitstatus = "0" ]; then
 if [ -z "$REPLACEME_SUBDOMAIN" ]; then
...
# Insert the data
sqlite $database "insert into $app (name,value,comment) values ('REPLACEME_SUBDOMAIN', '$REPLACEME_SUBDOMAIN', 'Subdomain for $app');"
...
sed -i "s/REPLACEME_SUBDOMAIN/$REPLACEME_SUBDOMAIN/g" docker-compose-final.yml
